...
export default function BasicTable() {
  ...
  let setDeleting = async (email) => {
    try {
      await axios
        .post("http://localhost:5000/deleteProf", {
          email,
        })
  ...
  let getData = async () => {
    try {
      await axios.get("http://localhost:5000/allProfs").then((res) => {
        let temp = res.data;
        for (let i of temp) {
          i.name = i.firstName + " " + i.lastName;
          i.nbrCours = i.courses.length;
          i.nbrFormations = i.formations.length;
        }
        setData(temp);
        setLoading(false);
        console.log("dataa\n", data, "\nres\n", res.data);
